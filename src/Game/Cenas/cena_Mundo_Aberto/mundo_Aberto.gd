...
#			$Fogopai/Fogo12.flip_h = false
	...
	if Mensagens.perguntaRegiaoAtual("prefeitura").respondido: #se ainda existirem perguntas na prefeitura sem resposta
		$AlertaAmarelo3.hide()
	else:
		$AlertaAmarelo3.show()

	if Mensagens.perguntaRegiaoAtual("escola").respondido: #se ainda existirem perguntas na escola sem resposta
		$AlertaAmarelo2.hide()
	else:
		$AlertaAmarelo2.show()

	if Mensagens.perguntaRegiaoAtual("mercado").respondido: #se ainda existirem perguntas no mercado sem resposta
		$AlertaAmarelo.hide()
	else:
		$AlertaAmarelo.show()
