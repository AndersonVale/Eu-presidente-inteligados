...
var data =  {
		...
		"industria" : {
			...
			"informacao" : "Isabel estava indo de carro para o trabalho e, ao se distrair, passou por um grande buraco na via, que furou seu pneu. Com isso, Isabel se atrasou para uma reunião importante do trabalho. Indignada com a condição de pavimentação da rua, Isabel questiona quem poderá se responsabilizar pelo ocorrido. Nessa situação, você responde:",
			"alternativas" : [
				"Isabel deverá entrar em contato com a União para requerer a correção da pavimentação da via pública que está com o buraco e furou seu pneu. No entanto, Isabel não poderá processar qualquer ente do Estado, pois o Estado não teve a intenção de causar qualquer prejuízo ao carro da Isabel em específico",
		...
		"mercado" : {
			"tipo" : Mensagens.Tipos.PERGUNTA,
			"informacao" : "Gilson não conseguiu pagar a última fatura e está devendo a sua operadora de celular. Ele recebe, com frequência, ligações de cobrança da operadora, que passou a ligar também para seus familiares para expor a condição de devedor do Gilson. Diante disso, Gilson, constrangido pela conduta da operadora, pergunta a você, Presidente, se tem algo que ele possa fazer para que a operadora pare de ligar para sua família. Como você aconselharia?",
			"alternativas" : [
		...
		"prefeitura" : {
			"tipo" : Mensagens.Tipos.PERGUNTA,
			"informacao" : "Reinaldo invade um shopping no Rio de Janeiro e ameaça explodi-lo com uma bomba que implantou em um lugar secreto, colocando diversas pessoas em perigo. No entanto, chega ao conhecimento da polícia que a bomba foi implantada por Reinaldo, o qual é preso para ser interrogado. Como ele não confessa, a polícia tem a ideia de torturá-lo para fazê-lo falar e, aproveitando que você está de passagem, questiona se poderiam prosseguir. Você responde:",
			"alternativas" : [
				...
				"Como a tortura seria uma prática para impedir a explosao da bomba, seria aceitável torturar Reinaldo para garantir que ele falasse o lugar da bomba antes que um mal maior acontecesse, uma vez que a Constituição Federal prioriza o interesse coletivo"
			],
			"explicacoes" : {
